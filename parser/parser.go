...
var precedences = map[token.TokenType]int{
	...
	token.LPAREN:   CALL,
	token.LBRACKET: INDEX, // index expression have the highest precedence of all.
}
...
func New(l *lexer.Lexer) *Parser {
	...
	// register parsing Array for token.LBRACKET
	...
	// right between the identifier and the list of arguments.
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		// If it finds such a function, it calls it, passing in the expression
		// returned by a prefixParseFn as an argument.
		p.nextToken()
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	// we need to check if it even exists.
...
// parseFunctionParameters constructs the slice of parameters by repeatedly
// building identifiers from the comma separated list.
...
// build an AST node *ast.PrefixExpression
...
// It uses this argument to construct an *ast.InfixExpression node,
// with left being in the Left field.
...
//  accepts an end parameter that tells the method
//  which token signifies the end of the list.
