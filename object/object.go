...
// Hashable can use this interface in our evaluator to check
// if the given object is usable as a hash key
// when we evaluate hash literals or index expressions for hashes.
type Hashable interface {
	HashKey() HashKey
}

// HashKey generate a hash key for an [*object.String, *object.Integer, *object.Boolean]
// that’s comparable and equal to the hash key of another
// [*object.String, *object.Integer, *object.Boolean] with the same .Value.
// But the hash keys for an *object.String must never be equal to
// the hash key for an *object.Integer or an *object.Boolean.
type HashKey struct {
	Type  ObjectType
	Value uint64
}
...
// HashPair hold  the original key object
// and the value object its mapped to.
